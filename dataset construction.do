...
* import data from MREID Gravity Portal https://www.usitc.gov/data/gravity/mreid.htm (Ahmad et al., 2023)
import delimited "mreid_public_release_1.0(1).csv", clear
*generate country pair identifier
egen country_pair = concat(iso3_o iso3_d) 
encode country_pair, gen(country_pair_encode)
rename operatingrevenueturnover OperatingrevenueTurnover
rename operatingrevenueturnover_green OperatingrevenueTurnover_green
rename operatingrevenueturnover_mergers OperatingrevenueTurnover_mergers
rename totalassetsthusd TotalassetsthUSD
rename totalassetsthusd_green TotalassetsthUSD_green
rename totalassetsthusd_mergers TotalassetsthUSD_mergers
rename numberofemployees Numberofemployees
rename numberofemployees_green Numberofemployees_green
rename numberofemployees_mergers Numberofemployees_mergers
rename fixedassetsthusd FixedassetsthUSD
rename fixedassetsthusd_green FixedassetsthUSD_green
rename fixedassetsthusd_mergers FixedassetsthUSD_mergers
...
*get real GDP from WDI -> as World Bank data changes, I only downloaded the data once from the World Bank (14/09/2024) and saved it as the data set "realGDP_constant2015USD_old", so this step is commanded out 
/*
...
*/
*get exchange rate data from WDI -> as World Bank data changes, I only downloaded the data once from the World Bank (14/09/2024) and saved it as the data set "xr_old", so this step is commanded out
/*
...
save xr.dta, replace
*/
* get Human Capital Index from PWT (Feenstra et al., 2015) https://www.rug.nl/ggdc/productivity/pwt/pwt-releases/pwt100 
...
* get bilateral XR regime data from Harms and Knaze (2021)
...
* get economic freedom index from Fraser Institute (2024), https://www.fraserinstitute.org/categories/economic-freedom
import excel "fraser.xlsx", firstrow clear
...
use fin_dev, replace //originally downloaded from https://data.imf.org/?sk=f8032e80-b36c-43b1-ac26-493c5b1cd33b&sid=1481126573525
rename code iso3_d
rename FD fd_host
...
merge m:1 iso3_d year using fraser 
drop if _merge == 2
drop _merge
drop Countries
...
*GDP
use realGDP_constant2015USD_old.dta, replace 
rename iso3_d iso3_o
rename ny_gdp_mktp_kd gdp_origin 
keep iso3_o gdp year
save realGDP_constant2015USD_origin.dta, replace
*merge by source country iso
use mreid_gdp.dta, replace
merge m:1 iso3_o year using realGDP_constant2015USD_origin.dta  
drop if _merge == 2
drop _merge
save mreid_gdp.dta, replace
...
use mreid_gdp, replace
merge m:1 iso3_o year using fraser
drop if _merge == 2
drop _merge Countries
save  mreid_gdp, replace
use fraser, replace // restore original naming of fraser dataset
rename freedom_o freedom
rename iso3_o iso3_d
save fraser, replace

* 2) bilateral gravity variables 
* use CEPII dataset, https://www.cepii.fr/cepii/en/bdd_modele/bdd_modele.asp
* name the downloaded datafile "Gravity_V202211_original.dta" to make it compatible with the naming used in this do-file
...
*outlier -> have been identified in the sector-level gravity regression later
...
save gravity_countrylevel_clean.dta, replace
...
*what else is needed in sector-level dataset?
...
*** here, I saved the final dataset for the sector-level regressions. FOr scrutiny, I have commented out this command and provide the datasets in the Zip-File.
save gravity_sectorlevel_clean.dta, replace

* generate total assets deflated by US GDP deflator (for robustness check later)

use gravity_sectorlevel_clean, replace
